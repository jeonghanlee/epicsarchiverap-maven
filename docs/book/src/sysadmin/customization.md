...
1. The `src/sitespecific` folder contains materials specific to your
   ...
   1. If this variable is not set, by default, we use the material in
      the `default` site.
   2. The files in the
   ...
   3. If you do not set the `ARCHAPPL_POLICIES` environment variable,
      ...
      2. Or you can unpack the WAR files within the container and
         ...
         deployment.
   4. If you do not set the `ARCHAPPL_APPLIANCES` environment
      variable, the `appliances.xml` is discovered in a similar
      fashion.
   5. There is also a site specific properties file called
...
As outlined in the [details](../developer/details.md) page, the archiver appliance
...
support for alternate type systems. Please
