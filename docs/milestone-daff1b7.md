...
Next session entry point: continue the M8 browser-to-HTTP rewrite (26 of 29 remain) by pattern batch, starting with the archive-and-poll workflow: build a GetUrlContent-plus-Awaitility helper, rewrite one representative, verify it on real Tomcat plus softIocPVX (source setEpicsEnv.bash; TOMCAT_HOME to a user Tomcat 9 with conf_original; package before the integration run; pkill -f eaatag=eaatesttm to clear leftovers), then convert the rest of that pattern. M4 (dependency refresh) follows M8.
