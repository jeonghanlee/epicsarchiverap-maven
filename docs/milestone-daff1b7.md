...
Next session entry point: the authorized failure corrections and full-profile verification are finished. The default, integration, and localEpics sets passed 749, 98, and 26 tests respectively, with zero failures, errors, or skips. Integration evidence is under /tmp/aa-m8-dbd-isolation.15cg2nm4/; default and localEpics evidence is under /tmp/aa-m8-complete.xrp4jyu7/. The next M8 work is committing the verified changes under separate Git authorization, reproducing test-compile and the default set in a fresh clone of that commit, and recording landing evidence. M8 remains In progress; M4 still waits on it.

Current M8 reinforcement checkpoint: every failure observed in the requested default and profile runs has been corrected and verified through the real code and fixtures. T9-T17 have passing focused evidence, and T2-T4 record the final complete-set passes. DbdArchiveTest preserved its three-event assertion and passed twice consecutively with independent temporary stores, then passed in the complete integration run at 22:41:17 PDT on 2026-09-15. The field/filter regression reproduced six failures before correction; the merged-store metadata regression reproduced five null-result errors before correction. Earlier failed runs remain available at the evidence paths in T3 and T9-T17. No commit or push has been performed for these changes.
...
- Stage 1 is the prerequisite of M4 (its T2 and T4 need compiled tests); M4 waits on it. EPICS fixtures launch softIocPVX directly (D20), not through epics-ioc-runner.
...
| T2 | 2026-09-15 17:23:13 PDT | JDK 21.0.12.1, Maven 3.9.9, Surefire 3.6.0; host execution, default selection and saved pom | Pass | ./mvnw -o -B test on the final source: 67 classes, 749 tests, zero failures/errors/skips; exit 0, 13 min 4 s. Maven and all archived XML totals agree. Includes all seven ETL partitions, property restoration, ten new field/filter cases, and five new real merged-store metadata cases. Evidence: /tmp/aa-m8-complete.xrp4jyu7/unit.log, unit-reports/, and verification.json. The prior 734-test passes remain under /tmp/aa-m8-final.gcsqxo7c/ and /tmp/aa-m8-remaining.4CxTb4/; the earlier ETL failures remain under /tmp/aa-m8-fixture-fix.fPMaiu/final-unit*. |
