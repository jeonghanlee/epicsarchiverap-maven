...
Next session entry point: the M8 browser-to-HTTP rewrite is done — 20 of 29 Selenium tests rewritten to mgmt BPL HTTP and each verified on real Tomcat 9.0.121 plus softIocPVX, the other 9 (multi-appliance cluster and large-volume) deleted as out of scope for aa's single-instance platform (D23), and the Selenium and webdrivermanager dependencies dropped. Remaining in M8: the reinforcement backlog on the kept tests (Thread.sleep to Awaitility, @TempDir, static ConfigServiceForTests isolation, JaCoCo), applied incrementally. M4 (dependency refresh) waits on M8. To run an integration test: source setEpicsEnv.bash; TOMCAT_HOME to a user Tomcat 9 with conf_original; package before the run; pkill -f eaatag=eaatesttm to clear leftovers.
